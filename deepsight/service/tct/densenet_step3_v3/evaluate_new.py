...
def run_model(net, loader, train=False, optimizer=None, gpu = True):
...
def interface(patches, probality_s, net, own_data_classes, cuda=True):
    ...
    try:
        predict_result, rnn_input = predict(seq, net, cuda = cuda)
    except Exception as e:
        raise e
    predict_result = F.softmax(predict_result)
    predict_result = torch.max(predict_result.data, dim=1)[1]
    predict_result = predict_result.data.cpu().numpy()
    predict_class = np.where(abs(predict_result-predict_result.max())<1e-10)[1][0]
    rnn_input = rnn_input.data.cpu().numpy()
    #print(rnn_input)
    return predict_class, predict_result , result_flag , rnn_input


def interface_new(patches, probality_s, net, own_data_classes, cuda=True):
    ind_to_class = dict(zip(range(len(own_data_classes)), own_data_classes))
    result_flag = 0
    if len(probality_s)==0:
        result_flag =1
        return 0, 1.0, result_flag, 0
    ...
    rnn_input = rnn_input.data.cpu().numpy()
    #print(rnn_input)
    return predict_class, predict_result.max() , result_flag , rnn_input

...
target_names = ['MIild', 'Moderate', 'Normal']
...
def train(criterion, num_train, epoch, writer, net, dataloader, optimizer, batch_size, lr_decay_period=100, check_point_dir='./cell/checkpoints', gpu=True):
	...
	for index_batch, (img, label) in enumerate(dataloader):
		...
		# epoch_fp += fp
		# epoch_fn += fn
		y_all = np.append(y_all, y.data.cpu().numpy())
		y_predict_all = np.append(y_predict_all, y_predict.cpu().numpy())
		#print(y_predict_all)
	...
	cls_report = classification_report(np.array(y_all), np.array(y_predict_all), target_names=target_names)
	writer.add_scalar('train_loss', loss.data[0], epoch)
	writer.add_scalar('train_err', epoch_err/train_total, epoch)
	#writer.add_scalar('classification_report',cls_report, epoch)
	...
	log_tmp = 'Train Epoch: {0:.1f}\t Loss: {1:.6f}\t Error: {2:.6f}\t' \
			  .format(epoch, epoch_loss/train_total, epoch_err/train_total)
	print(log_tmp)
	print(cls_report)
	file_name = 'log_3cls'
	...
	if epoch%5 == 0:
		torch.save(net.state_dict(), check_point_dir + '/CP{}.pth'.format(epoch))
...
if __name__ == '__main__':
	...
	parser.add_argument('--lr', dest='lr', help='learning rate', type=float, default=0.00001)
